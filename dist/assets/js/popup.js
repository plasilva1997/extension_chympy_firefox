...
browser.storage.local.get("token").then(getTokenExist, onError);
...
function login() {
    ...
    fetch(urlAPI+"particuliers/login", { //requete avec les données
        ...
        if(data['success'] !== false) {
            ...
            browser.storage.local.set({token_at:d.toString()})
                .then(setItem, onError);
            getCompany();
        }else{
...
function getCompany(){
        ...
        token=browser.storage.local.get("token").then(getToken, onError);
        console.log(token);
